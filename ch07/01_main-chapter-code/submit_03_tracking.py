...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="W&B server. Defaults to the public cloud (personal account). "
             "Point elsewhere for a self-hosted instance.",
    )
    parser.add_argument(
        "--wandb_config_dir",
        type=str,
        default="~/.config/wandb-personal",
        help="W&B credential/config directory, kept separate from "
             "~/.config/wandb so a work login is never disturbed.",
    )
    parser.add_argument(
        "--wandb_entity",
        type=str,
    ...
    if args.no_wandb:
        ...
        # Pin the host and credential directory for THIS run, so a
        # WANDB_API_KEY or base_url inherited from the shell cannot silently
        # redirect personal runs to a work instance.
